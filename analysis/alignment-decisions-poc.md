...
|---|-------|-------------|-------|--------|
| 1 | DAPR HTTP Middleware | ⚠️  **VALIDATION FAILED**: Attempted real DAPR integration with sidecar + Redis pub/sub. Critical Finding: DAPR's built-in HTTP middleware type (`middleware.http.transformation`) is NOT a registered component type in daprd. DAPR sidecar crashed with "HTTP middleware middleware.http.transformation/v1 has not been registered" error. **Implication:** Built-in DAPR middleware path is blocked. Must pivot to **Sidecar Adapter Pattern**: custom middleware container between DAPR and service, configured to intercept HTTP. This adds complexity but is the validated working approach. | TBD | BLOCKED - REQUIRES ARCHITECTURE PIVOT |
