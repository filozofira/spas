...
- `add-inventory-item` - `POST /inventory/items`
  ...
  - Parameters: `productId` (required), `initialQuantity` (optional, defaults to 0)
  - Returns: 201 Created with inventory item details
  - Conflict: 409 if product already exists in inventory

**Added Event:**
- `inventory-item-added` (v1.0) - Emitted when inventory tracking initialized
  - Fields: `productId`, `initialQuantity`, `timestamp`
...
`initialQuantity` defaults to 0 for flexibility. Rationale: Products can be cataloged before stock arrives, or initial stock can be set immediately.
