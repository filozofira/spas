...
- ADR-035: Simplified CLI publish workflow (Dec 2025): Consolidate `spas-service metadata get`, `pack`, and `publish` into single `publish` command with `--dry-run` and `--archive` flags. SDKs generate a complete metadata ZIP archive offline (e.g., `service.metadata.zip`), eliminating the need for separate pack steps or service metadata endpoints. `--dry-run` enables inspection without publishing (replaces metadata get). `--archive` enables CI/CD pipelines to publish pre-built archives without running a service. Rationale: Reduces developer friction, standardizes on a single supported artifact, maintains full workflow coverage with fewer commands.
...
- ADR-037: AI-in-the-loop composition for spas-compose (Dec 2025): Choreography composition uses AI agent (`/spas.compose` prompt) for iterative contract analysis and transformation generation. CLI provides scaffolding (`init`), metadata retrieval (`services pull`), and building deployment artifacts (`choreography build`); AI assists with semantic matching of event contracts, proposing topic mappings, and generating JSONata transformation files. Developer confirms each step via iterative prompt loop. Rationale: Choreography composition involves semantic understanding unsuited for deterministic CLI; leverages GitHub SpecKit patterns; reduces command complexity while improving accuracy.
...
- ADR-039: JSON Schema draft-07 standard (Dec 2025): All SPAS metadata schemas (design-time, runtime) and service contract schemas use JSON Schema draft-07. Rationale: Broad validator support (Ajv for Node.js, JsonSchema.Net for .NET); stable specification with excellent tooling; avoids draft-04 legacy issues and draft 2019-09/2020-12 adoption gaps. SDK emits draft-07, Repository validates draft-07, CLI references draft-07 schemas. Future schema formats (Avro, Protobuf) may be added via pluggable validation (schema registry extensibility).
- ADR-040: Schema property naming uses camelCase (Dec 2025): SDK-generated JSON schemas use camelCase property names to match runtime JSON serialization behavior. Rationale: .NET services use `System.Text.Json` with `JsonNamingPolicy.CamelCase` at runtime; schemas must reflect actual wire format for accurate validation and transformation authoring. All SDK must use camelCase policy. Applies to event, command, and query schemas.
