...
describe('DockerGenerator', () => {
  ...
  beforeEach(() => {
    ...
    fs.mkdirSync(path.join(servicesDir, 'order-service', 'schemas'), { recursive: true });
    fs.mkdirSync(path.join(servicesDir, 'fulfillment-service', 'schemas'), { recursive: true });

    // Create minimal service metadata
    fs.writeFileSync(
      path.join(servicesDir, 'order-service', 'spas.json'),
      JSON.stringify({ id: 'order-service', version: '1.0.0', network: { port: 5001 } })
    );
    fs.writeFileSync(
      path.join(servicesDir, 'fulfillment-service', 'spas.json'),
      JSON.stringify({ id: 'fulfillment-service', version: '1.0.0', network: { port: 5002 } })
    );
  });
