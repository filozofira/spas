...
public static class MetadataEndpointExtensions
{
    ...
    /// <summary>
    /// Maps the /_spas/metadata endpoint with auto-generated schemas from type attributes.
    /// </summary>
    /// <param name="app">The web application.</param>
    /// <param name="metadataProvider">Function that provides spas.json content.</param>
    /// <param name="assemblyToScan">Optional assembly to scan for types. If null, scans the entry assembly.</param>
    /// <returns>The web application for chaining.</returns>
    public static WebApplication MapSpasMetadataEndpoint(
        this WebApplication app,
        Func<object> metadataProvider,
        Assembly? assemblyToScan = null)
    {
        ...
        // Map active endpoint with auto-generated schemas
        app.MapGet(options.Path, async (HttpContext context) =>
        {
            var archiveWriter = app.Services.GetRequiredService<MetadataArchiveWriter>();
            var metadata = metadataProvider();

            // Auto-generate schemas from types with SPAS attributes
            var targetAssembly = assemblyToScan ?? Assembly.GetEntryAssembly()!;
            var schemaGenerator = new SchemaGenerator();
            var schemas = await schemaGenerator.GenerateSchemasFromAssemblyAsync(targetAssembly);
