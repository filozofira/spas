...
2. Use `<name>` to construct paths: `./examples/domains/b2b/subscription/<name>/...`
...
**Domain root**: `./examples/domains/b2b/subscription`
**Full domain path**: `./examples/domains/b2b/subscription/{DOMAIN}/`
...
1. **Contract Analysis**: Parse service metadata from `./examples/domains/b2b/subscription/{DOMAIN}/services/*/spas.json`
...
```
./examples/domains/b2b/subscription/{DOMAIN}/
...
The choreography.yaml flows generate sidecar configuration files. Use the schema at `./examples/domains/b2b/subscription/{DOMAIN}/.spas/schemas/sidecar-config-v1.schema.json` to understand the mapping:
...
1. **Validate Workspace**
   - Verify `./examples/domains/b2b/subscription/{DOMAIN}/choreography.yaml` exists
   - Verify `./examples/domains/b2b/subscription/{DOMAIN}/services/` directory exists with at least one service
   - If invalid: Show error and suggest `spas-compose init {DOMAIN} --output ./examples/domains/b2b/subscription`, then `spas-compose services pull`

2. **Read Service Contracts**
   - Read `./examples/domains/b2b/subscription/{DOMAIN}/services/<service-name>/spas.json` for each service
   - Extract: `id`, `version`, `boundedContext`, `endpoints[]`, `events[]` (outbound only)
   - Read schemas from `./examples/domains/b2b/subscription/{DOMAIN}/services/<service-name>/schemas/`
...
1. **Create Transformation Files**
   - Generate JSONata files at `./examples/domains/b2b/subscription/{DOMAIN}/transformations/<service>/*.jsonata`
...
| Constraint | Behavior |
|------------|----------|
| **Read-only services/** | NEVER modify files in `./examples/domains/b2b/subscription/{DOMAIN}/services/` |
...
| No choreography.yaml | "Error: Workspace not initialized. Run `spas-compose init {DOMAIN} --output ./examples/domains/b2b/subscription` first." |
